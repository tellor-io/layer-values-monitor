...
def simulate_catchup_process(start_height: int, end_height: int) -> None:
    ...
    for i in range(0, total_events, BATCH_SIZE):
        ...
        print(f"🔄 Processing batch {i//BATCH_SIZE + 1}: heights {min(batch_heights)}-{max(batch_heights)} ({len(batch)} events)")
        ...
        for event_data in batch:
            ...
            print(f"  📤 Queued event from height {event_data['height']} with query_id {event_data['attributes']['new_report.query_id'][0][:12]}...")
