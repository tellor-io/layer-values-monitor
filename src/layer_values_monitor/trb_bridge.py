...
def decode_query_data(query_data: str) -> Optional[int]:
...
def decode_report_value(value_hex: str) -> Optional[Tuple[str, str, int, int]]:
...
async def get_trb_bridge_trusted_value(
    ...
    try:
        ...
        elif os.getenv("TRBBRIDGE_RPC_URL"):
            # Use RPC endpoint from environment
            w3 = Web3(Web3.HTTPProvider(os.getenv("TRBBRIDGE_RPC_URL")))
        else:
        ...
        try:
            ...
            logger.info(f"Retrieved deposit details - Sender: {sender}, Layer_Addr: {layer_recipient}, Amount: {amount}, Tip: {tip}, Block Number: {block_number}")
